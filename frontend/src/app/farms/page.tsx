...
import { useFarms, useSearchFarms, useCreateFarm } from '@/hooks/useFarms'
import {
  Plus,
  Search,
  MapPin,
...
export default function FarmsPage() {
  ...
  const { data: allFarms, isLoading: allFarmsLoading, error: allFarmsError } = useFarms()
  const { data: myFarms, isLoading: myFarmsLoading, error: myFarmsError } = useFarms({ my: true })
  const { data: searchResults, isLoading: searchLoading, error: searchError } = useSearchFarms(searchTerm)
  const createFarmMutation = useCreateFarm()

  // Determine which data to use
  const farms = searchTerm ? searchResults : (showMyFarms ? myFarms : allFarms)
  const loading = searchTerm ? searchLoading : (showMyFarms ? myFarmsLoading : allFarmsLoading)
  const error = searchTerm ? searchError : (showMyFarms ? myFarmsError : allFarmsError)
  ...
  const handleDeleteFarm = async () => {
    ...
    try {
      ...
      // Invalidate farms queries to refresh the data
  ...
  return (
    <div className="container mx-auto px-4 py-8" dir="rtl">
      ...
      <PageNavigation
        title="إدارة المزارع"
        subtitle="أضف وأدير مزارع الأسماك الخاصة بك"
        actions={[
          {
            label: 'إضافة مزرعة',
            icon: Plus,

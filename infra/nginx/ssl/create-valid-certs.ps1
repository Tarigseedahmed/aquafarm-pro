# Create valid SSL certificates for AquaFarm Pro
param(
    [string]$Domain = "localhost"
)

Write-Host "Creating valid SSL certificates for AquaFarm Pro..." -ForegroundColor Green
...
try {
    ...
    $cert = New-SelfSignedCertificate -Subject "CN=$Domain" -DnsName @($Domain, "localhost", "127.0.0.1", "*.localhost") -CertStoreLocation "Cert:\CurrentUser\My" -NotAfter (Get-Date).AddDays(365) -KeyAlgorithm RSA -KeyLength 2048 -KeyUsage DigitalSignature, KeyEncipherment -TextExtension @("2.5.29.37={text}1.3.6.1.5.5.7.3.1,1.3.6.1.5.5.7.3.2")
    ...
    # Export private key using RSA format
    $rsa = $cert.PrivateKey
    if ($rsa -ne $null) {

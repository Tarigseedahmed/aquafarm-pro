...
- اختبار آلي للبث SSE (قادم Sprint 2).
- Retry-After header للـ 429 (تحسين لاحق).

**ما يمكن تحسينه:**

- إدخال ESLint Markdown workflow لتقليل وقت إصلاح التنسيق اليدوي.
- اختبار تلقائي للبث SSE (محاكاة إنشاء إشعار).
- تفعيل caching بسيط للترجمة code→tenantId.
- Named throttler مستقل للاستخدامات الخاصة (login) بدلاً من override على global.
- توحيد DTO للصفحات (Generic Paginated\<T\> schema في OpenAPI).
...
4. إضافة cache اختياري (in-memory Map) لترجمة tenant code.
5. تحسين تغطية الاختبارات: strict mode + failure cases + SSE stream.
6. إضافة Retry-After header للـ 429 وثروتلر باسم مستقل.
7. توثيق Paginated\<T\> عام وإعادة استخدامه في Swagger.
8. تصميم واجهة WebSocket مبدئية (Gateway) اختيارية.

...
| المرحلة 1: التصميم المعماري + شريحة Backend أولى | 🔄 جاري التنفيذ | 15% | 25 سبتمبر 2025 | 15 أكتوبر 2025 | AI Assistant |
...
| 9 | تحديث التوثيق (README + خطة) | Docs | 0.5ي | Backend | جاري (OpenAPI تحديث مستمر) |
...
- Interceptor للتينانت وفّر نقطة مركزية لعزل البيانات وتقليل تكرار المنطق في الخدمات.
- إضافة بارامتر Swagger عالمي حسّن تجربة الاختبار اليدوي وخفّض أخطاء نسيان الهيدر.
- اختبارات العزل E2E (فلاح المزرعة / الأحواض / إحصائيات المزرعة) أعطت ثقة مبكرة بعدم تسرّب بيانات.
...
- إضافة اختبار تلقائي إضافي يتحقق من رفض إنشاء مزرعة بدون X-Tenant-Id عند تفعيل TENANT_STRICT (اختبار موجود جزئياً؛ توسيع التغطية مطلوب).
- تفعيل caching بسيط للترجمة code→tenantId لتقليل استعلامات قاعدة البيانات عند الضغط.

**المخاطر المكتشفة:**

- تباين تنسيقات الوثائق (Markdown lint) قد يبطئ المراجعات مستقبلاً.
- الحاجة المبكرة لتصميم RBAC قبل توسع نطاق الوحدات.

### الإجراءات المقترحة Sprint 2

1. بدء Water Quality module (entities + CRUD + tenant isolation).
2. تحسين UsersService (scoped listing) + اختبار سلبي cross-tenant.
3. تقديم طبقة Authorizations مبكرة (permissions matrix بسيطة JSON).
4. إضافة cache اختياري (in-memory Map) لترجمة tenant code خلال lifespan للطلب أو TTL قصير.
5. تحسين تغطية الاختبارات: strict mode + failure cases.

...
export class TenantsService implements OnModuleInit {
  ...
  async onModuleInit() {
    // Temporarily disable to allow app to start
  ...
  async create(dto: CreateTenantDto): Promise<Tenant> {
    ...
    const created = await this.tenantRepo.save(this.tenantRepo.create(dto));
    this.tenantCache.invalidate(created.id);
    this.tenantCache.invalidate(created.code);
    return created;
  }
  ...
  async update(id: string, dto: UpdateTenantDto): Promise<Tenant> {
    ...
    this.tenantCache.invalidate(saved.id);
    this.tenantCache.invalidate(saved.code);
    return saved;
  }
  ...
  async remove(id: string): Promise<void> {
    ...
    this.tenantCache.invalidate(tenant.id);
    this.tenantCache.invalidate(tenant.code);
  }
}

...
export class NotificationsService implements OnModuleInit {
  constructor(
    ...
    @Optional() private readonly redisNotifications?: RedisNotificationsService,
  ) {
  ...
  async onModuleInit() {
    ...
    if (this.redis?.isEnabled() && this.redisNotifications) {
      // Use the dedicated Redis notifications service
      await this.redisNotifications.subscribeToUserNotifications(
        '*',
        '*',
    ...
    } else if (this.redis?.isEnabled()) {
      // Fallback to direct Redis subscription
  ...
  // Redis Pub/Sub methods for distributed notifications
  async publishNotification(notification: Notification) {
    if (this.redis?.isEnabled() && this.redisNotifications) {
      // Use the dedicated Redis notifications service
      await this.redisNotifications.publishNotification(notification);
    } else if (this.redis?.isEnabled()) {
      // Fallback to direct Redis publish
      const payload = { ...notification, __origin: this.instanceId };
      await this.redis.publish('notifications.created', payload);
    }
  }
  ...
  async subscribeToNotifications(callback: (notification: Notification) => void) {
    if (this.redis?.isEnabled() && this.redisNotifications) {
      // Use the dedicated Redis notifications service
      await this.redisNotifications.subscribeToUserNotifications('*', '*', callback);
    } else if (this.redis?.isEnabled()) {
      // Fallback to direct Redis subscription
      await this.redis.subscribe('notifications.created', (payload: any) => {
        if (!payload) return;
  ...
  async create(notificationData: Partial<Notification>, tenantId?: string): Promise<Notification> {
    ...
    await this.publishNotification(saved);
    return saved;
  }

...
### Metrics (Prometheus)
- Endpoint: `GET /metrics` (content-type: `text/plain; version=0.0.4`)
- Includes default Node.js/NestJS process metrics and custom app metrics:
  - `http_requests_total{method,status}`
  - `http_request_duration_seconds_bucket{method,route,status,le}` (+ `_sum`/`_count`)
  - `http_5xx_errors_total{route,status}`
  - `rate_limit_exceeded_total{route}`
  - `forbidden_requests_total{route,reason}`
  - `unauthorized_requests_total{route}`
  - `sse_clients_total` and `active_sse_connections`

### Grafana Dashboard
- Import Prometheus as a data source and create panels using the metrics above.
- Suggested panels:
  - Request rate (RPS) by route and status
  - p50/p90/p99 latency
...
1. Start Prometheus, Grafana, and Backend:
   - `docker compose -f docker-compose.dev.yml up -d prometheus grafana backend-dev`
2. Grafana at http://localhost:3002
   - Login: admin / admin123
   - Data source: Prometheus → URL: `http://prometheus:9090`
   - Import dashboard JSON: `infra/grafana/dashboards/aquafarm-observability.json`
...
```bash
./scripts/backup-postgres.sh localhost 5432 aquapro_dev postgres ./backups
```

Set `PGPASSWORD` env var or edit the script for your credentials. In production, use managed backups or cron + secure storage (S3/MinIO).

See also: `docs/backup-restore.md` for the full plan and DR checklist.
